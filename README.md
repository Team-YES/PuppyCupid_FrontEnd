...
- 최초 로그인 시 전화번호 미등록 유저에게는 임시 토큰(temp_access_token)을 발급하여 추가 정보 입력을 유도하고,
  등록 완료 시 정식 토큰(access, refresh)을 재발급합니다.
- refresh_token 요청 시 DB에 저장된 값과 일치하는 경우에만 access 토큰을 재발급합니다.
- Axios 요청 시, 별도로 설정한 axiosInstance를 통해 쿠키를 자동으로 포함하도록 구성하여 인증이 필요한 요청에서도 일관된 방식으로 백엔드 API와 통신할 수 있도록 설계하였습니다.
- withCredentials: true 옵션과 공통 baseURL, Authorization 헤더 설정을 포함하여, 반복적인 설정 없이 보안이 필요한 요청을 효율적으로 처리하였습니다.

### 👥 유저 접속 권한 (이용권 유저 / 블랙리스트 / 일반 유저)

- 유저 유형에 따라 접근 가능한 페이지를 구분하여 보안성과 사용자 경험을 높였습니다.
- 유저 타입은 3가지로 분류되며, 각 타입에 따라 접근 가능한 라우트를 다르게 설정하였습니다.
  
  - 이용권 유저: 결제를 완료한 유저. 프리미엄 기능 및 특정 페이지 접근 가능. (산책메이트, MBTI 매칭도 이용 가능)
  - 블랙리스트 유저: 제재된 유저. 대부분의 페이지 접근 불가, 리다이렉트 처리됨. (게시판 보기만 가능, 댓글 작성 및 기타 활동 불가)
  - 일반 유저: 비회원 또는 로그인은 했지만 결제하지 않은 유저. 일부 기본 기능만 사용 가능. (유료 기능을 제외한 페이지만 이용 가능)
...
- 메시지 전송 시 첫 메시지에 한해 알림(notification)을 보여주고, 알림을 읽으면 읽음 상태를 보내 상태를 저장합니다.
...
- 알림을 읽으면 fetch로 읽음 표시를 백엔드로 보내서 상태를 저장합니다.
